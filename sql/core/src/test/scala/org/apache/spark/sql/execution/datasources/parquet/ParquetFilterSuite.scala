...
abstract class ParquetFilterSuite extends QueryTest with ParquetTest with SharedSparkSession {
  ...
  test("SPARK-39393 Do not push down filters when field is of type repeated primitive") {
    ...
    withTempDir { dir =>
      ...
      // If the "isnotnull(f)" filter gets pushed down, this query will throw an exception since
      // column "f" is repeated primitive column in the actual Parquet file.
      checkAnswer(
